...
public class Page5 {
	...
	public void confirmPage(String payment_settings, String room_name, boolean cc_owner) throws IOException {
		...
		try {
			...
			if(!file_has_reservationDetails_label()){
				get_label_list = labels.get_confirmPage_labels(); 
				write_csv_reservationDetails(get_label_list);
			}
			get_values_list = values.get_confirmPage_values(payment_settings, room_name, cc_owner);
	...
	private void write_csv_reservationDetails(ArrayList<HashMap<Integer, String>> data) throws IOException {
		for(int cell_row = 0; cell_row < data.size(); cell_row++){
			for(int cell_column = 0; cell_column < 20; cell_column++){
				add_backlash_character(get_values_list);
	...
	private void add_backlash_character(ArrayList<HashMap<Integer, String>> data) throws IOException {
